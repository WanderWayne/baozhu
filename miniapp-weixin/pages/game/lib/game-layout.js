...
const WORKSHOP_ITEM_SIZE = 85;
const WORKSHOP_EDGE_MARGIN = 8;
const WORKSHOP_INVENTORY_GAP = 10;
...
/** 物品栏增高时，把落入新底边的合成区物品沿向上的窄锥形推出。 */
function nudgeWorkshopItems(items, synthesisWidth, synthesisHeight, random = Math.random) {
  if (!Array.isArray(items) || !synthesisWidth || !synthesisHeight) {
    return { items: Array.isArray(items) ? items : [], changed: false };
  }

  const maxX = Math.max(WORKSHOP_EDGE_MARGIN, synthesisWidth - WORKSHOP_ITEM_SIZE - WORKSHOP_EDGE_MARGIN);
  const maxY = Math.max(WORKSHOP_EDGE_MARGIN, synthesisHeight - WORKSHOP_ITEM_SIZE - WORKSHOP_INVENTORY_GAP);
  let changed = false;
  ...
  const next = items.map((item) => {
    ...
    const depth = item.y - maxY;
    const halfConeRad = Math.PI / 8;
    const theta = ((random() * 2) - 1) * halfConeRad;
    const distance = depth + 18 + random() * 32;
    const nextX = Math.min(
      Math.max(WORKSHOP_EDGE_MARGIN, item.x + Math.sin(theta) * distance),
      maxX,
    );
    const nextY = Math.min(
      Math.max(WORKSHOP_EDGE_MARGIN, item.y - Math.cos(theta) * distance),
      maxY,
    );
    changed = true;
