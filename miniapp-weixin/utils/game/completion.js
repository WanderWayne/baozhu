...
module.exports = {
  ...
  showSettlementScreen: function() {
    return new Promise((resolve) => {
      const elapsed = levelManager.getBasicLevelElapsedTime();
      const timeStr = levelManager.formatElapsedTime(elapsed);
      const rating = levelManager.getSpeedRating(elapsed);
      const completed = (levelManager.currentProgress.completedLevels || [])
      ...
      this.page.setData({
        ...
        settlementTime: timeStr,
        settlementRatingIcon: rating.icon,
        settlementRatingName: rating.name,
        settlementRatingColor: rating.color,
        settlementGems: gems,
      });
