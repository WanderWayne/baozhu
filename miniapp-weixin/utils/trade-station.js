...
class TradeStationManager {
  ...
  _buildViews(levelId) {
    ...
    const invTop = inv ? inv.top : (synth ? synth.bottom : 0);
    ...
    if (!centered && synth) {
      ...
      offsetY = midY - synthTop - totalH / 2;
    }
    ...
    return this.stations.map((s, i) => {
      ...
      const topPx = centered ? 0 : offsetY + i * (stationH + gap);
      const stationStyle = centered
        ? `top:50%;transform:translateY(-50%) scale(${scale});`
        : `top:${topPx}px;transform:scale(${scale});`;
      const layoutLeft = synth ? synth.width - STATION_RIGHT_PAD - STATION_BOX_W : 0;
      const layoutTop = centered
        ? (synth ? synth.height / 2 - STATION_BOX_H / 2 : 0)
        : topPx;
      return {
        id: s.id,
        input: s.input,
  ...
  dismissConfirm(playSound = true) {
    if (playSound) {
      try { this.page.audioManager.playSFX('click-back'); } catch (err) { /* noop */ }
    }
  ...
  _spawnOutput(s) {
    ...
    let spawnX = 48;
    let spawnY = 120;
    if (stationView && stationView.top != null) {
      spawnX = 56;
      spawnY = Math.max(40, stationView.top + 8);
    }
  ...
  updateProximityFromItem(item, touchX, touchY) {
    ...
    this.stations.forEach((s) => {
      ...
      const dTouch = (touchX != null && touchY != null)
        ? Math.hypot(touchX - sc.x, touchY - sc.y)
        : Infinity;
      const near = Math.min(dItem, dTouch) < TRADE_HIT_RADIUS;
      if (s.hover !== near) {
        s.hover = near;
        changed = true;
      }
