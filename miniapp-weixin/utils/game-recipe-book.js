...
function buildRecipeBookList(tab, query, discoveredItems) {
  ...
  if (tab === 'recorded') {
    ...
    if (!entries.length) {
      return {
        emptyText: q ? '没有找到匹配的配方' : '暂无记载',
        sections: [],
      };
    }

    const sections = [];
    const ingredients = entries.filter((e) => e.category === 'ingredient');
    const beverages = entries.filter((e) => e.category === 'beverage');
    if (ingredients.length) sections.push({ label: '小料', entries: ingredients });
    if (beverages.length) sections.push({ label: '饮品', entries: beverages });
    return { emptyText: '', sections };
  }
  ...
  if (!entries.length) {
    return {
      emptyText: q ? '没有找到匹配的配方' : '还没有发现任何配方',
      sections: [],
    };
  }
...
function getRecipeBookPageData(tab, query, discoveredItems) {
  const list = buildRecipeBookList(tab, query, discoveredItems);
  return {
    recipeBookTab: tab,
