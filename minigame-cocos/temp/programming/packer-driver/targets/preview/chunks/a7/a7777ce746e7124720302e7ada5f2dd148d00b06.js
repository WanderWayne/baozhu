      ...
      _export("GameScene", GameScene = (_dec = ccclass('GameScene'), _dec(_class = class GameScene extends Component {
        ...
        onEnable() {
          ...
          router.onChange((scene, routeState) => {
            ...
            var levelId = routeState.levelId || 101;
        ...
        enterLevel(levelId) {
          ...
          var level = ctx.config.getLevels().find(l => l.id === levelId);
        ...
        trySynthesize(itemA, itemB, extraItem) {
          ...
          var items = [itemA, itemB];
          if (extraItem) items.push(extraItem);
          var result = ctx.recipes.synthesize(items);
        ...
        tryUnlockFragment(itemName) {
          ...
          var fragment = ctx.config.getFragments().find(f => f.trigger === itemName);
